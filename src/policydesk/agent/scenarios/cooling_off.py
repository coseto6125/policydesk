...
COOLING_OFF = Scenario(
    ...
    injection=(
        ...
        "這個送達日期系統裡沒有存，你也不知道。"
        "所以在保戶說出收到保單的日期之前，你不能算出撤銷期限是哪一天，也不能說「已經過期了」或「還來得及」——"
        "只能照 verbatim 說明期間是幾天、從什麼時候起算，然後反問保戶是什麼時候實際收到保單的。\n"
        "保戶說出收到日期之後，期限用 date_calculations 算，不要在腦中數日子："
        "翌日起算的意思是收到當天不算、隔天是第一天，所以末日就是「收到日 + verbatim 裡的天數」，"
        "例如 3 月 1 日收到、期間十日，寫成 2026-03-01 + 10 日，末日是 3 月 11 日。"
        "回覆裡說這個日期是依他所說的收到日算出來的，送達日以保險公司的送達紀錄為準。"
